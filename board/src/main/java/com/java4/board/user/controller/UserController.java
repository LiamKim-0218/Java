...
public class UserController {
	...
	public String registPost(@RequestParam Map<String, String> map, Model model) {
		 try {
		        ...
		        if (userId == null || !userId.matches("[a-zA-Z0-9]{3,20}")) {
		            throw new IllegalArgumentException("유효하지 않은 아이디입니다. 아이디는 3자에서 20자까지의 영문 또는 숫자로 이루어져야 합니다.");
		        }

		        String password = map.get("password");
		        if (password == null || !password.matches("(?=.*[a-zA-Z])(?=.*\\d)(?=.*[!@#$%^&])[a-zA-Z\\d!@#$%^&]{10,30}")) {
		            throw new IllegalArgumentException("유효하지 않은 비밀번호입니다. 비밀번호는 대소문자, 숫자, 특수문자(!@#$%^&)를 포함하여 10자에서 30자까지여야 합니다.");
		        }

		        String name = map.get("name");
		        if (name == null || (!name.matches("[가-힣]{2,10}") && !name.matches("[a-zA-Z]{4,20}"))) {
		            throw new IllegalArgumentException("유효하지 않은 이름입니다. 이름은 한글 2자에서 10자 또는 영문 4자에서 20자까지로 이루어져야 합니다.");
		        }

		        String phone = map.get("phone");
		        if (phone == null || !phone.matches("(\\d{3}-\\d{4}-\\d{4})|(\\d{10,11})")) {
		            throw new IllegalArgumentException("유효하지 않은 전화번호입니다. 전화번호는 'XXX-XXXX-XXXX' 또는 'XXXXXXXXXX' 형식이어야 합니다.");
		        }

		        String address = map.get("address");
		        if (address != null && !address.matches(".{5,100}")) {
		            throw new IllegalArgumentException("유효하지 않은 주소입니다. 주소는 5자에서 100자까지로 이루어져야 합니다.");
		        }

		        String email = map.get("email");
		        if (email == null || !email.matches(".+@.+\\.(com|org|co\\.kr|net)")) {
		            throw new IllegalArgumentException("유효하지 않은 이메일입니다. 유효한 이메일 주소를 입력하세요 (.com, .org, .co.kr, .net으로 끝나야 함).");
		        }

		        String gitAddress = map.get("git-address");
		        if (gitAddress != null && !gitAddress.matches("https://github.com/[a-zA-Z0-9]+")) {
		            throw new IllegalArgumentException("유효하지 않은 Git 주소입니다. 유효한 GitHub 주소를 입력하세요 ('https://github.com/'로 시작해야 함).");
		        }

		       
		        User tempUser = new User(userId, password, name, phone, email);
		        if (address != null) {
		            tempUser.setAddress(address);
		        }
		        if (gitAddress != null) {
		            tempUser.setGitAddress(gitAddress);
		        }
